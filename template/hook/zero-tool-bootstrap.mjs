...
export function apply(ctx, config) {
  ...
  const bootstrapPersonaText = typeof config?.bootstrapPersonaText === 'string' && config.bootstrapPersonaText.length > 0
    ? config.bootstrapPersonaText
    : undefined
  ...
  ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
    ...
    try {
      ...
      if (boundary < 0) return applyBootstrapPrompt({ ...assembled, tools: [] })
      // Post-compaction: core work set so mid-task work can continue.
      if (compactionTools.length === 0) return applyBootstrapPrompt({ ...assembled, tools: [] })
      const available = new Set(assembled.tools.map((tool) => tool.name))
