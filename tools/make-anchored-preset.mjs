...
/** Optional whoami anchor plugins (upstream whoami-standard). */
export const WHOAMI_HOOK_FILES = ['whoami-turn.mjs', 'zero-tool-bootstrap.mjs']
...
export const WHOAMI_HOOK_SOURCES = Object.fromEntries(
  WHOAMI_HOOK_FILES.map((file) => [file, new URL(`../template/hook/${file}`, import.meta.url)]),
)
...
/**
 * Render the whoami-anchor rows (upstream whoami-standard flow): the first
 * model request sees a fixed self-introduction prompt on an EMPTY tool
 * surface, that reply promotes the session, and the real message runs on the
 * NEXT turn with the resident catalog. `includeSubagents: true` makes
 * subagents inherit the same anchor flow, so they also arrive at the resident
 * catalog instead of starting on the full standard dump.
 */
export function buildWhoamiRows({
  text = '你是谁',
  suppressedContextSources = ['agent-instructions', 'skill-catalog'],
  suppressedContextPlugins = [],
  bootstrapPersonaText = undefined,
  ...
  if (bootstrapPersonaText !== undefined) config.push(`    bootstrapPersonaText: ${yamlDouble(bootstrapPersonaText)}`)
  if (compactionTools.length > 0) config.push(`    compactionTools: ${yamlList(compactionTools)}`)
  config.push('    includeSubagents: true')
  return [
    '# ── whoami anchor (generated by dsh-anchored-standard; keep this row FIRST) ──',
    '# The first request sees only the fixed whoami prompt on an EMPTY tool',
    '# surface; that reply promotes the session and the real message runs next',
    '# turn on the resident catalog. Subagents inherit the same anchor flow.',
...
export async function generateAnchoredPreset(options) {
  ...
  const whoamiRow = buildWhoamiRows({
    suppressedContextSources: resolved.suppressedContextSources,
    suppressedContextPlugins: resolved.suppressedContextPlugins,
    bootstrapPersonaText: resolved.bootstrapPersonaText,
    compactionTools: resolved.compactionTools,
  ...
  if (whoami) {
    for (const file of WHOAMI_HOOK_FILES) {
      await writeFile(join(targetDir, file), await readFile(WHOAMI_HOOK_SOURCES[file], 'utf8'))
    }
  }
  await writeFile(
...
if (isMain) {
  try {
    ...
    if (options.help === true) {
      ...
      if (plan.whoami) process.stdout.write('anchor flow: whoami-standard (zero-tool first turn; subagents inherit it)\n')
