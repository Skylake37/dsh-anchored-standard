 ...
 */
export function buildCompanionRows({ promoteOn = 'either', includeSubagents = false } = {}) {
  return [
...
/**
 * Remove `complete: true` from a source `@deepseek-ai/dsh-persona` row.
 *
 * DSH's `system-prompt/assemble` captures a registered complete section
 * BEFORE the waterfall and restores it AFTER every listener, so a source
 * preset whose persona row is `complete: true` silently defeats the
 * anchor-bootstrap persona swap. The generator must make the row ordinary so
 * the hook's phase-aware `deployment:persona` replacement is authoritative.
 */
export function stripPersonaComplete(composition) {
  const lines = composition.replace(/\r\n/g, '\n').split('\n')
  const nameIndex = lines.findIndex(line => /^\s*name\s*:\s*['"]?@deepseek-ai\/dsh-persona['"]?\s*(?:#.*)?$/.test(line))
  if (nameIndex === -1) return { composition, stripped: false }
  let start = nameIndex
  while (start > 0 && !/^\s*-\s*id:/.test(lines[start - 1])) start--
  let end = nameIndex + 1
  while (end < lines.length && !/^\s*-\s*id:/.test(lines[end])) end++
  let stripped = false
  for (let index = end - 1; index >= start; index--) {
    if (/^\s*complete\s*:/.test(lines[index])) {
...
export async function generateAnchoredPreset(options) {
  ...
  const companionRows = buildCompanionRows({ promoteOn: resolved.promoteOn, includeSubagents: resolved.subagents !== 'resident' })
